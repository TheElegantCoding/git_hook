...
const lintStaged = () => {
  try {
    ...
    logger.success('All staged files have been validated and corrected.', { blankAbove: true, blankBelow: true });
  } catch {
    logger.error('Critical error while fetching Git files.', { blankAbove: true, blankBelow: true });
    process.exit(1);
  }
};
